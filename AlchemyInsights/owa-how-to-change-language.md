---
title: Alterar idioma, formato de data e hora ou fuso horário no Outlook na Web
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000014"
- "2000"
ms.openlocfilehash: 84c3086d2be1376254b10e702ae61d8ea13a372cbae6eee874562a8c1235566d
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/05/2021
ms.locfileid: "54010568"
---
# <a name="change-your-language-date-and-time-format-or-time-zone-in-outlook-on-the-web"></a>Alterar seu idioma, formato de data e hora ou fuso horário no Outlook na Web

1. Na parte superior da página, selecione <img src='data:image/png;base64,' />
 **Configurações**  >  **Exibir todas as Outlook configurações**.
2. Selecione **Idioma Geral** e  >  **hora**.
3. Selecione o idioma que você deseja usar.
4. Selecione **Salvar**.

Para saber mais, confira [Alterar o fuso horário](https://support.office.com/article/65239869-12e7-4a9d-bca1-76b0ad7ce273)e as configurações de idioma em Outlook na Web .