---
title: '8000019: como adicionar ou importar contatos'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 8e9535da046f21cc35ac9704492d8cd04e770aa997a6995f00dd3953241e471b
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/05/2021
ms.locfileid: "53999768"
---
# <a name="import-contacts"></a>Importar contatos

Antes de importar contatos para Outlook na Web, você precisará exportá-los de seu outro provedor de email como um arquivo .csv de email. Em seguida, siga as instruções abaixo.

1. Na parte inferior da página, selecione <img src='data:image/png;base64,' />
 **Pessoas**.
2. Na barra de ferramentas, selecione **Gerenciar**  >  **Importar contatos**.
3. Selecione **Procurar**, escolha seu arquivo CSV e selecione **Abrir**.
4. Selecione **Importar**.

Para saber mais, confira [Importar contatos para Outlook](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

